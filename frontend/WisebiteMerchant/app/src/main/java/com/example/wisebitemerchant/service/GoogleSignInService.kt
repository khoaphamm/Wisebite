...
class GoogleSignInService(private val context: Context) {
    ...
    // Using the same WEB Client ID from Google Cloud Console as the customer app
    // This is the same ID used in the backend .env file
    ...
    suspend fun signIn(): GoogleSignInResult {
        return try {
            ...
            Log.e("MerchantGoogleSignIn", "GetCredentialException: ${e.message}", e)
            GoogleSignInResult(
                success = false,
                errorMessage = "Đăng nhập Google thất bại: ${e.message}"
            )
        } catch (e: CancellationException) {
            Log.e("MerchantGoogleSignIn", "Sign-in was cancelled", e)
            GoogleSignInResult(
                success = false,
                errorMessage = "Đăng nhập đã bị hủy"
            )
        } catch (e: Exception) {
            Log.e("MerchantGoogleSignIn", "Unexpected error during sign-in", e)
            GoogleSignInResult(
                success = false,
    ...
    private fun handleSignInResult(result: GetCredentialResponse): GoogleSignInResult {
        return try {
            ...
            if (credential.type == GoogleIdTokenCredential.TYPE_GOOGLE_ID_TOKEN_CREDENTIAL) {
                ...
                Log.e("MerchantGoogleSignIn", "Unexpected credential type: ${credential.type}")
                GoogleSignInResult(
                    success = false,
            ...
            Log.e("MerchantGoogleSignIn", "Invalid Google ID token", e)
            GoogleSignInResult(
                success = false,
                errorMessage = "Token Google không hợp lệ"
            )
        } catch (e: Exception) {
            Log.e("MerchantGoogleSignIn", "Error parsing credential", e)
            GoogleSignInResult(
                success = false,
